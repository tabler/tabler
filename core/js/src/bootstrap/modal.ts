...
class Modal extends BaseComponent {
  ...
  declare _cancelHandler: (event: Event) => void
  declare _closeHandler: () => void
  ...
  _closeAndCleanup(): void {
    // Set before close() so the `close` listener below sees this as a close
    // it already knows about, not an external one it needs to clean up after.
    this._isTransitioning = true
    this._element.close()
    this._isTransitioning = false
  ...
  _addEventListeners(): void {
    ...
    // The native `close` event fires whenever the dialog closes by any means
    // other than hide() — a `<form method="dialog">` submit, a direct
    // close() call, or a non-cancelable Escape (Chrome closes immediately
    // without a recent user activation). Clean up the state hide() would
