...
(function () {
    ...
    var global$3 = hugerte.util.Tools.resolve('hugerte.Resource');

    var global$2 = hugerte.util.Tools.resolve('hugerte.util.I18n');

    const pLoadHtmlByLangCode = (baseUrl, langCode) => global$3.load(`hugerte.html-i18n.help-keynav.${ langCode }`, `${ baseUrl }/js/i18n/keynav/${ langCode }.js`);
    ...
    const tab$1 = editor => {
      ...
      const getPluginData = (editor, key) => find(urls, x => {
        ...
        const html = makeLink({
          name: x.name,
          url: `https://www.hugerte.org/docs/hugerte/1/${ x.slug }/`
        });
        return {
          name: x.name,
          html
      ...
      const htmlPanel = {
        type: 'htmlpanel',
        presets: 'document',
        html: [installedPlugins(editor)].join('')
      };
      return {
        name: 'plugins',
        title: 'Plugins',
