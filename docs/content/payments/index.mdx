...
If your project uses [React](/payments/react), [Vue](/payments/vue), [Preact](/payments/preact) or [Astro](/payments/astro), you can use payment provider logos as components instead of CSS classes. Each provider ships as its own tree-shakable component (for example `PaymentVisa`), plus a `Payment` component that renders by slug — useful for a data-driven list of accepted methods.
