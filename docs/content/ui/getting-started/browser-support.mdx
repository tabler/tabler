...
Relative colors, `oklch(from …)`|The darker hover shade of every color and the dark mode borders|122|128|16.4

Opera 109 and Samsung Internet 27 are the first releases built on Chromium 123.

Safari 16.4 to 17 follow an older draft of relative colors, where math on the hue needs a `deg` unit. Tabler only changes the lightness, which works the same in every version. If you override a Tabler color with `light-dark()`, its `-darken` shade needs Safari 18.
