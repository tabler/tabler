...
| Navbar theme | `data-bs-navbar-theme` | `default` | `default`, `dark` |
| Sidebar | `data-bs-sidebar` | `default` | `default`, `folded`, `folded-hover` |
| Sidebar theme | `data-bs-sidebar-theme` | `default` | `default`, `dark` |
...
The last six settings change the page layout rather than its colors, and they need the markup described in [Page layouts](/ui/layout/page-layouts):
...
- `default` on the two theme settings means "follow the page color mode", not "always light". Set `dark` to give the navbar or the sidebar the dark palette on a light page - the same result as putting `data-bs-theme="dark"` on that element in your markup.
