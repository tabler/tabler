...
css-plugins: [themes]
related: [/ui/getting-started/customize, /ui/getting-started/color-modes]
---

import CdnImportPlugin from '@components/CdnImportPlugin.astro';

Every gray you see in Tabler - card backgrounds, borders, muted text, table stripes - is built from a `--tblr-gray-50` … `--tblr-gray-950` scale. By default that scale is the `neutral` palette. The `tabler-themes` stylesheet adds four more takes on the same scale - `slate`, `gray`, `zinc`, and `stone` - so you can pick a cooler or warmer neutral without touching a single component's CSS.
...
Set `data-bs-theme-base` on `<html>` to one of five values. `neutral` is the default - Tabler looks the same with or without the attribute set to `neutral`.

| Value | Description |
| --- | --- |
| `neutral` | Tabler's default pure, desaturated gray (no attribute needed) |
| `slate` | A cooler, slightly blue-tinted gray |
| `gray` | A cool gray with a subtle blue tint |
...
<Info title="The stylesheet alone changes nothing">
