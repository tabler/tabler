...
summary: Tabler draws charts with ApexCharts and ships a container class, a color palette and theme tokens that make them match the interface.
...
ApexCharts draws its own legend, but you can also build a plain-HTML legend next to a chart's title - for example to label the series above the chart instead of below it. Use the [Legend](/ui/components/legend) component: a `legend-list` of `legend` elements, colored with `legend-{color}`.

<Example>
<div class="legend-list">
  <span class="legend legend-primary"><span class="legend-dot"></span> This year</span>
  <span class="legend legend-secondary"><span class="legend-dot"></span> Last year</span>
</div>
</Example>
