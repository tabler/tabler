...
### Dark navbar and sidebar

```html
<html data-bs-navbar-theme="dark">
<html data-bs-sidebar-theme="dark">
```

`default` means "follow the page color mode", so the navbar is light on a light page and dark on a dark one. `dark` gives that element the dark palette on a light page, exactly like putting `data-bs-theme="dark"` on the navbar in your markup:

```html
<header class="navbar navbar-expand-md" data-bs-theme="dark">...</header>
```

A light navbar inside a dark page is not supported - a light-scoped subtree inside a dark page inherits the dark styles.
