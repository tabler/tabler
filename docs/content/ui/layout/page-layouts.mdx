...
The attribute covers whichever navigation the page shows. The sidebar gives up its own width for the gap, so the page next to it stays in place. On the two-row navbar both rows float. The navigation floats only from its `navbar-expand-*` breakpoint up. Without the attribute, add the [`navbar-floating`](/ui/layout/navbars#floating-navbar) class to a navbar in your markup:
