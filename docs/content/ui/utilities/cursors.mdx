---
title: Cursors
summary: Cursor utilities set the mouse pointer for an element, so the cursor shows what happens on click or drag.
description: Change the mouse cursor with utility classes to signal what users can do with an element, from pointers to resize handles.
related: [/ui/utilities/interactions]
---
