...
<div class="row row-cols-1 sm:row-cols-2 lg:row-cols-3 xl:row-cols-6 g-3">
  {colors.map((color) => (
  <div class="col">
    <div class={`border rounded bg-gradient bg-gradient-from-${color} mb-2 p-4`}></div>
...
<Example codeOnly>
  {colors.map((color) => (
  <div class={`bg-gradient bg-gradient-from-${color}`}>...</div>
  ))}
</Example>
