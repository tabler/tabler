...
classnames:
  ...
  modifier:
    ...
    - class: modal-fullscreen
      desc: Fills the viewport; {breakpoint}:modal-fullscreen-down only below that width
  size:
    - class: modal-sm
      desc: Small dialog
    - class: modal-lg
...
- `data-bs-keyboard="false"` can't be fully guaranteed in Chrome: without a recent user activation on the page, Chrome's close watcher makes the native `cancel` event non-cancelable, so Escape closes the dialog immediately no matter what.
