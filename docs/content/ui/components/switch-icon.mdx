...
Tabler toggles the `active` class on every element with `data-bs-toggle="switch-icon"` on click, after dispatching the `tabler:switch-icon-toggle` event described above. This is the code that runs:
