...
classnames:
  ...
  style:
    ...
    - class: accordion-button-toggle-plus
      desc: Plus indicator that turns into a close icon when open
  modifier:
...
Use `.accordion-button-toggle-plus` with a `plus` icon. When the section opens, the toggle rotates a quarter turn and the plus becomes a close icon. It rotates the whole toggle, so it works with an inline SVG, a sprite `<use>` or an icon font.
