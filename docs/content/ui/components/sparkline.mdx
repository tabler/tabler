...
Call `update()` with new values to redraw the chart. It writes the values back to `data-bs-values` and fires `updated.bs.sparkline`:
...
- Color alone should not carry meaning. For signed bars the negative values also hang below the zero line, which is what makes them readable.
