...
related: [/ui/plugins/vector-map]
source: core/scss/_utilities.scss
classnames:
  component:
    - class: ratio
      desc: Wrapper that keeps the embed at a fixed aspect ratio
  size:
    - class: ratio-{ratio}
      desc: The aspect to hold, for example ratio-16x9 or ratio-21x9
---
...
A Mapbox account and an access token are needed, because every map load calls the Mapbox API. If you only need a simple map of countries or regions, use the [vector map](/ui/plugins/vector-map) instead. It needs no account and no network calls.
...
<div class="ratio ratio-16x9">
  <div><div id="map-simple" class="w-100 h-100"></div></div>
</div>
</Example>
...
<div class="card">
  <div class="card-body">
    ...
    <div class="ratio ratio-16x9">
      <div><div id="map-card" class="w-100 h-100"></div></div>
    </div>
  </div>
</div>
</Example>
...
<div class="card">
  <div class="ratio ratio-21x9">
    <div><div id="map-full" class="w-100 h-100 rounded"></div></div>
  </div>
</div>
</Example>
