---
title: Dropdown
summary: A dropdown is a menu that opens from a button or a link and holds options or actions, so the toolbar stays short.
...
An [icon](/ui/components/icon) in front of each item makes the options faster to scan.
...
A [badge](/ui/components/badge) in an item shows a count or a status next to it.
...
Items can hold [checkboxes](/ui/forms/elements), which makes a dropdown work as a filter.
...
A menu can hold [card](/ui/components/card) content, such as a short profile or a summary, when a plain list isn't enough.
