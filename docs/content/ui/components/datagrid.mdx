...
Use the `.datagrid` class to build a responsive list of label and value pairs. Each `.datagrid-item` contains a `.datagrid-title` and a `.datagrid-content`, which can hold text, [avatars](/ui/components/avatar), [statuses](/ui/components/status), or [form controls](/ui/forms/elements).
