...
- A status dot is decoration. The state has to be in text next to it - a screen reader ignores a coloured circle, and so does anyone who cannot tell the colours apart.
- Do not use red and green alone to separate "down" from "up". Pair the colour with a word, an icon, or a shape.
