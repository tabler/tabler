...
Use the `.datagrid` class to build a responsive list of label and value pairs. Each `.datagrid-item` contains a `.datagrid-title` and a `.datagrid-content`, which can hold text, avatars, statuses, or form controls.
