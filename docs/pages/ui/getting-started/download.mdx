...
Tabler uses other packages to enhance its functionality - for example, [charts](/ui/components/chart) and [input masks](/ui/forms/form-input-mask). These are not automatically installed to avoid huge dependency trees and need to be installed by using `npm install`. The full list of recommended libraries is available on the
