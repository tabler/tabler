...
const illustrations = globSync(join(repoRoot, 'shared/static/illustrations/light/*.png'))
  .map((file) => basename(file, '.png'))
  .sort()

writeFileSync(join(repoRoot, 'shared/data/illustrations.json'), JSON.stringify(illustrations))
